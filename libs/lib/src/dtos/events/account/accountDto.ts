...
export class AccountDto extends PartialAccountDto{
    ...
    constructor(account: Account){
        ...
        this.address = account.addresses ? account.addresses.map(address => (new AddressDto(address))) : [];
        this.store = account.stores ? account.stores.map(store => (new StoreDto(store))) : [];
    }
}